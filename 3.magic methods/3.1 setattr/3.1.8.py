...
class GeyserClassic:
    ...
    def __init__(self):
        self.attrs = ('Mechanical', 'Aragon', 'Calcium')
        self.slots = {(1, self.attrs[0]): None, (2, self.attrs[1]): None, (3, self.attrs[2]): None}
    ...
    def remove_filter(self, slot_num):
        if type(slot_num) is int and 1 <= slot_num <= 3:
            key = (slot_num, self.attrs[slot_num - 1])
    ...
    def water_on(self):
        return all(self.slots.values()) and all(
            0 <= time.time() - i.date <= self.MAX_DATE_FILTER for i in self.slots.values())
